...
final class BizCity_CRM_Bridge_Zalo {
	...
	public static function send_text( string $token, string $user_id, string $text ): array {
		// [2026-07-07 Johnny Chu] HOTFIX — bridge-level outbound trace for CRM debug.
		$_trace = ( isset( $GLOBALS['_bizcity_channel_send_trace'] ) && is_array( $GLOBALS['_bizcity_channel_send_trace'] ) )
			...
			: ( isset( $GLOBALS['_bizcity_outbound_trace_ctx'] ) && is_array( $GLOBALS['_bizcity_outbound_trace_ctx'] )
				? (array) $GLOBALS['_bizcity_outbound_trace_ctx']
				: array() );
		error_log( sprintf(
			'[CRM Zalo Bridge TRACE] send_text trace=%s source=%s uid=%s len=%d hash=%s',
			(string) ( $_trace['trace_id'] ?? '-' ),
			(string) ( $_trace['source'] ?? 'crm.bridge.zalo' ),
			$user_id,
			mb_strlen( $text ),
			substr( sha1( $text ), 0, 12 )
		) );
	...
	public static function send_image( string $token, string $user_id, string $image_url ): array {
		...
		$_trace = ( isset( $GLOBALS['_bizcity_channel_send_trace'] ) && is_array( $GLOBALS['_bizcity_channel_send_trace'] ) )
			...
			: ( isset( $GLOBALS['_bizcity_outbound_trace_ctx'] ) && is_array( $GLOBALS['_bizcity_outbound_trace_ctx'] )
				...
				: array() );
		error_log( sprintf(
			'[CRM Zalo Bridge TRACE] send_image trace=%s source=%s uid=%s image=%s',
			(string) ( $_trace['trace_id'] ?? '-' ),
			(string) ( $_trace['source'] ?? 'crm.bridge.zalo' ),
			$user_id,
			$image_url !== '' ? 'yes' : 'no'
