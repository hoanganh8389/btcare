...
class BizCity_CRM_Adapter_Zalo extends BizCity_CRM_Adapter_Base {
	...
	public function send( array $conversation, array $message ): array {
		...
		if ( ctype_digit( $ref ) && class_exists( 'BizCity_Gateway_Sender' ) ) {
			...
			// [2026-07-07 Johnny Chu] HOTFIX — explicit source stamp for outbound trace.
			$res     = BizCity_Gateway_Sender::instance()->send(
				$chat_id,
				$payload,
				$content_type === 'image' ? 'image' : 'text',
				array( '_trace_source' => 'crm.adapter.zalo' )
			);
	...
	// [2026-06-21 Johnny Chu] PHASE-0.39 GURU-BIND — private→protected so BizCity_CRM_Adapter_ZaloOA can inherit.
	protected function resolve_uid_from_conversation( array $conversation ): string {
		global $wpdb;
