<?php
/**
 * [2026-06-07 Johnny Chu] PHASE-0.43 M5 — DDV probe: CRM Broadcast Mass-Send Deplao Parity 2
 *
 * Probe ID  : core.crm.broadcast_deplao
 * Order     : 46
...
if ( class_exists( 'BizCity_Probe_CRM_Broadcast_Deplao', false ) ) {
	return;
}

final class BizCity_Probe_CRM_Broadcast_Deplao implements BizCity_Diagnostics_Probe {

	public function id(): string    { return 'core.crm.broadcast_deplao'; }
	public function label(): string { return 'CRM Broadcast Mass-Send (PHASE-0.43 Deplao Parity 2)'; }
	// [2026-06-08 Johnny Chu] HOTFIX — add missing interface methods (description/severity/icon/estimate_ms).
	public function description(): string { return 'Kiểm tra schema, dispatcher, REST route và cron hook cho broadcast mass-send Deplao (PHASE-0.43 M5).'; }
	...
	public function tags(): array   { return array( 'crm', 'broadcast', 'deplao', 'cron', 'phase-0.43' ); }
	...
	public function run( $ctx ): array {
		...
		$json_path = BIZCITY_TWIN_AI_PATH . 'core/diagnostics/changelog/modules.twin-crm.json';
		$json_ok   = false;
		$json_ver  = 'n/a';
		...
		$dispatcher_file = BIZCITY_TWIN_AI_PATH . 'plugins/bizcity-twin-crm/includes/campaigns/class-broadcast-dispatcher.php';
		...
		$adapter_file = BIZCITY_TWIN_AI_PATH . 'plugins/bizcity-zalo-personal/includes/class-zalo-personal-adapter.php';
...
add_filter( 'bizcity_diagnostics_register_probes', static function ( array $probes ): array {
	$probes[] = new BizCity_Probe_CRM_Broadcast_Deplao();
	return $probes;
} );
